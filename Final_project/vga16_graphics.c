...
void drawPixelDither(short x, short y, char color1, char color2) {
    ...
    if (pixel & 1) {
      if((x & 1) & (y & 1))
        vga_data_array[pixel>>1] = (vga_data_array[pixel>>1] & TOPMASK) | (color1 << 4) ;
      else
        vga_data_array[pixel>>1] = (vga_data_array[pixel>>1] & TOPMASK) | (color2 << 4) ;
    }
    else {
        vga_data_array[pixel>>1] = (vga_data_array[pixel>>1] & BOTTOMMASK) | (color2) ;
    }
}
